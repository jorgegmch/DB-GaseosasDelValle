...
-- Script: gdv_queries.sql
-- Descripción: Consultas complejas.
...
-- Consulta 9: Promedio de valor de pedido por sede

SELECT
    s.nombre_sede,
    COUNT(p.id_pedido) AS total_pedidos,
    AVG(p.total_con_iva) AS promedio_valor_pedido
FROM sedes s
LEFT JOIN pedidos p ON s.id_sede = p.id_sede
GROUP BY s.id_sede, s.nombre_sede
ORDER BY promedio_valor_pedido DESC;

-- Consulta 10: Productos con precio por encima del promedio general
